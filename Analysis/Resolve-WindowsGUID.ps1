...
.EXAMPLE
Resolve-WindowsGUID.ps1 -GUID 724EF170-A42D-4FEF-9F26-B60E846FBA4F
AdminTools
...
[Parameter(Mandatory=$True,ValueFromPipeLine=$True,Position=0)]
    ...
    Begin {
        ...
        # Static entries availabe on the web
        $WinGUIDHT = @{
        ...
        & $env:windir\system32\logman.exe query providers | ForEach-Object {
            ...
            if ($provider -match "\{") {
                ...
                if ($WinGuidHT.ContainsKey($LogGuid)) {} else {
                    $WinGuidHT.Add($LogGuid, $LogName)
                }
            }
        }
    ...
    Process {
        ...
        if ($HumanReadableValue = $WinGUIDHT[$guid]) {
            $HumanReadableValue
        } else {
            "Not found."
        }
